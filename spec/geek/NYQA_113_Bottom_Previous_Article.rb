...
describe "Verify Bottom Previous Article on Article Page" ,:selenium => true,:retry => 3 do
  before(:all) do
  ...
  it "Verify the Bottom Previous Article Link can be navigated to from the Article page" do
    @Template.clickandwait(@selenium.find_element(:css,"article.right h3>a"))
    $link_text = @selenium.find_element(:css,"div.paginationbottom>div.previous>a").text
    $href_val = @Template.getAttribute("css","div.paginationbottom>div.previous>a","href")
    @selenium.execute_script("document.getElementById('outbrain_widget_1').scrollIntoView()")
    @Template.clickandwait(@selenium.find_element(:css,"div.paginationbottom>div.previous>a"))
  end
