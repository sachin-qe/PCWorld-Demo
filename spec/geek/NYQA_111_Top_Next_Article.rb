...
describe "Verify Top Next Article on Article Page" ,:selenium => true,:retry => 3  do
  before(:all) do
  ...
  it "Verify the Top Next Article Link can be navigated to from the Article page" do
    @Template.clickandwait(@selenium.find_element(:css,"article.right h3>a"))
    $link_text = @selenium.find_element(:css, "div.pagenationtop>div.next>a").text
    $href_val = @Template.getAttribute("css","div.pagenationtop>div.next>a","href")
    @Template.clickandwait(@selenium.find_element(:css,"div.pagenationtop>div.next>a"))  	
  end
