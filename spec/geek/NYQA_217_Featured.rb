...
describe "Verify Featured_Link1" ,:selenium => true,:retry => 3  do
  before(:all) do
    @selenium = RSpec.configuration.selenium
    @Template = TemplatePage.new(@selenium)
    @wait = @Template.set_timeout(30)
    @selenium.get("#{@Template.get_Base_URL}/featured/") 
  end  
  ...
  it"Verify Goes to correct URL" do
    @selenium.current_url.should == "#{@Template.get_Base_URL}/featured/"
  end

  it"Verify Page Title is correct"do
    @Template.verifyText("css","title","Featured News & Rumors | Tag Page | Geek.com")
  end
  ...
  it "Verify The page is populated with articles related to Featured category " do
