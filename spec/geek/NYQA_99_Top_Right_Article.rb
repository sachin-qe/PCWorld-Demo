...
describe "Verify Top Right Article from Homepage" ,:selenium => true,:retry => 3 do
  before(:all) do
  ...
  it "Verify it goes to the article page" do
    $link_title = @selenium.find_element(:css,"article.right h3>a").text
    $href_val = @Template.getAttribute("css","article.right h3>a","href")
  ...
  it "Verify TOP link exists and check it actually goes to the top" do
