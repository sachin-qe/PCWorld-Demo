...
describe "Verify The Buzz page", :selenium => true,:retry => 3 do
  before(:all) do
  ...
  it "Verify click on The Buzz from Footer link" do
    @lib_obj.waitforpagetoload
    @selenium.find_element(:css, ".lb-columns>ul>li>a[href*='press']").click
    @lib_obj.waitforpagetoload
  end
  ...
  it "Verify the page heading LOGICBUY PRESS exists" do
    @doc = Nokogiri::HTML(open(@selenium.current_url))
    @doc.css("#about_header").text.should == "LogicBUY Press"  
    #@selenium.find_element(:xpath,"//*[@id='about_header']").text.should == "LOGICBUY PRESS"  
  end

  it "Verify Page content exists"do
    @selenium.find_element(:css,".content_info").displayed?.should == true   
  end
